...
public class Ya : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        for (int x = 0; x < m_InputSize.x; x++)
        {
            for (int y = 0; y < m_InputSize.y; y++)
            {
                for (int z = 0; z < m_InputSize.z; z++)
                {
                    ...
                    Gizmos.DrawSphere(transform.position + new Vector3(x, y + .5F, z), .25F);
                }
            }
        }
        ...
        Gizmos.DrawWireCube(transform.position + new Vector3(m_InputSize.x - 1, m_InputSize.y, m_InputSize.z - 1) / 2, new Vector3(m_InputSize.x, m_InputSize.y, m_InputSize.z));
    }
}
